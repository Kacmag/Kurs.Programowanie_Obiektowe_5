...
namespace PzG {
 ...
 void LaczeDoGNUPlota::WyswietlajKomuniangleyBledow( bool  WyswietlajKomuniangley )
 {
  _WyswietlajKomuniangleyOBledach =  WyswietlajKomuniangley;
 }
 ...
 bool LaczeDoGNUPlota::PrzeslijDoGNUPlota(const char *Komuniangle)
 {
  int Ilosc = strlen(Komuniangle);
  int IloscPrzeslanych;
  
  while (((IloscPrzeslanych = write(_Wejscie_GNUPlota,Komuniangle,Ilosc)) != -1)
          && Ilosc > 0) {
    Komuniangle += IloscPrzeslanych;
    Ilosc -= IloscPrzeslanych;
  }
  if (Ilosc > 0) {
    KomuniangleBledu("!!! Przeslanie polecenia do GNUPlota nie powiodlo sie.");
    return false;
  }
  return true;
 }
...
/*
                                      ...
                                      )
 {
  ...
  if (!CzyJestPlik(NazwaPliku)) {
    string Komuniangle = "!!! Ostrzezenie: Brak pliku ";
    Komuniangle += NazwaPliku;
    //TMP    KomuniangleBledu(Komuniangle.c_str());
    rezultat = false;
  }
 ...
 */
 ...
 void LaczeDoGNUPlota::KomuniangleBledu(const char *Komuniangle) const
 {
  if (!CzyWyswietlacKomuniangley()) return;
  cerr << Komuniangle << endl;
 }
 ...
 */
 bool LaczeDoGNUPlota::UtworzProcesPotomny()
 {
  ...
  switch (fork()) {
    case -1: KomuniangleBledu("### Niemozliwe rozwidlenie procesu.");
             exit(1);
    case  0: 
       /* Start potomka */

       if (close(STDIN) == -1) { 
         KomuniangleBledu("### Blad przy zamykaniu strumienia wejsciowego.");
         exit(1);
       }
       if (dup(outdesk[STDIN]) == -1) {
         KomuniangleBledu("### Blad duplikacji kanalu wejsciowego.");  
         exit(1);
       }
       ...
       if (dup(indesk[STDOUT]) == -1) {
         KomuniangleBledu("### Blad duplikacji kanalu wyjsciowego.");  
         exit(1);
       }       
       if (close(outdesk[STDIN]) == -1) {
         KomuniangleBledu("### Blad zamkniecia kanalu wejsciowego.");
         exit(1);
       }
       if (close(outdesk[STDOUT]) == -1) {
         KomuniangleBledu("### Blad zamkniecia kanalu wyjsciowego.");
         exit(1);
       }
       if (close(indesk[STDIN]) == -1) {
         KomuniangleBledu("### Blad zamkniecia dupliangleu kanalu wejsciowego.");
         exit(1);
       }
       if (close(indesk[STDOUT]) == -1) {
         KomuniangleBledu("### Blad zamkniecia dupliangleu kanalu wyjsciowego.");
         exit(1);
       }
       char Tab[3];
       read(STDIN,Tab,1);
       ...
       KomuniangleBledu("!!! Blad:"); 
       KomuniangleBledu("!!! W procesie potomnym nie mogl zostac."); 
       KomuniangleBledu("!!! uruchomiony program gnuplot."); 
       KomuniangleBledu("!!! Nastapilo przerwanie dzialania procesu potomnego."); 
       KomuniangleBledu("!!! Jednym z mozliwych powodow problemu moze byc"); 
       KomuniangleBledu("!!! Brak programu gnuplot w szukanej sciezce."); 
       KomuniangleBledu("!!! Do uruchomienia programu gnuplot bylo uzyte polecenie:");
       KomuniangleBledu(Comm);
       exit(1);

    default:
       if (close(outdesk[STDIN]) == -1 || close(indesk[STDOUT]) == -1) { 
         KomuniangleBledu(" Blad zamkniecia outpipe[STDIN], inpipe[STDOUT]."); 
       }
 ...
 bool LaczeDoGNUPlota::RysujDoPliku(const char *NazwaPliku)
 {
  ...
  if (!PrzeslijDoGNUPlota(Polecenie.c_str())) {
    KomuniangleBledu("!!! Rysunek nie moze byc utworzony.");
    return false;
  }

  bool Wynik = Rysuj();
  if (!PrzeslijDoGNUPlota("\n\n\nset output \nset term x11\n")) {
    KomuniangleBledu("!!! Wyjscie dla gnuplot'a nie zostalo poprawnie\n"
 ...
 */
 ...
 void LaczeDoGNUPlota::XZrotationSet( float KatX_st, float KatZ_st )
 {
